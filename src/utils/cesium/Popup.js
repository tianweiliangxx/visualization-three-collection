...
export default class Popup {
  ...
  add(conf) {
    ...
    if (conf.isclose === false) {
      ...
      if (
        ctn.getElementsByClassName('bx-popup-close') &&
        ctn.getElementsByClassName('bx-popup-close').length > 0
      ) {
  ...
  render() {
    ...
    for (var c in _this.ctnList) {
      ...
      if (position && position.x && position.y) {
        if (
          Math.abs(position.x) > window.innerWidth * 2 ||
          Math.abs(position.y) > window.innerHeight * 2
        ) {
          _this.ctnList[c][1].style.display = 'none'
        } else {
          _this.ctnList[c][1].style.display = ''
          _this.ctnList[c][1].style.left = position.x + 200 + 'px'
          _this.ctnList[c][1].style.top = position.y + 60 + 'px'
        }
      }
    }
  }
