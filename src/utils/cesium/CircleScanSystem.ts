...
export default class CircleScanSystem {
  private viewer: Cesium.Viewer
  private data: CircleScanOptions
  private primitives: Cesium.PostProcessStage | null
  constructor(viewer: Cesium.Viewer, options: CircleScanOptions) {
    this.viewer = viewer
    const data = {
 ...
 *添加圆形扩大扫描圈
 返回当前圆形扩大扫描圈对象
 * */
function addCircleScan(viewer: Cesium.Viewer, data: CircleScanOptions) {
 ...
 *添加区域雷达扫描
 返回当前雷达扫描对象
 * */
function addRadarScan(viewer: Cesium.Viewer, data: CircleScanOptions) {
  viewer.scene.globe.depthTestAgainstTerrain = true //防止移动、放大缩小会视觉偏移depthTestAgainstTerrain // 设置该属性为true之后，标绘将位于地形的顶部；如果设为false（默认值），那么标绘将位于平面上。缺陷：开启该属性有可能在切换图层时会引发标绘消失的bug。
  const CartographicCenter = new Cesium.Cartographic(
    Cesium.Math.toRadians(data.lon),
    Cesium.Math.toRadians(data.lat),
